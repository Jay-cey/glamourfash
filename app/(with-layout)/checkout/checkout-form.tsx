...
function PaymentForm({ orderId }: { orderId: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      >
        {isProcessing ? "Processing..." : <><FaLock className="w-4 h-4" /> Pay Now</>}
      </button>
    </form>
  );
}

function AddressForm({ user, onOrderCreated }: { user: any, onOrderCreated: (secret: string, orderId: string) => void }) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.success && result.clientSecret && result.orderId) {
        onOrderCreated(result.clientSecret, result.orderId);
      } else {
...
export default function CheckoutForm({ user }: CheckoutFormProps) {
  ...
  const handleOrderCreated = (secret: string, id: string) => {
        ...
        <div className="lg:grid lg:grid-cols-12 lg:gap-x-12 lg:items-start">
          ...
          <section className="lg:col-span-7">
            {clientSecret ? (
              ...
              <AddressForm user={user} onOrderCreated={handleOrderCreated} />
            )}
          </section>
